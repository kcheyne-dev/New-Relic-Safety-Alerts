...
function hasOfficeHeadcounts() {
  return OFFICES.some(o => o.headcount != null);
}
function fmtHeadcount(h) {
  return h == null ? '—' : Number(h).toLocaleString();
}
function sumHeadcount(offices) {
  return offices.reduce((s, o) => s + (o.headcount || 0), 0);
}
...
function aggregateAcledRisk(countryNames) {
...
function normalizeWhoCountry(name) {
  return WHO_COUNTRY_ALIASES[name] || name;
}

function hasWhoOutbreaks() {
  return WHO_OUTBREAKS.length > 0;
}
function outbreaksForCountry(countryName) {
  return WHO_OUTBREAKS.filter(o => normalizeWhoCountry(o.country) === countryName);
}
function outbreaksAggregated(countryNames) {
  const all = [];
  for (const name of countryNames) {
...
function alertCountryFor(alert) {
 ...
 */
function relevanceTierOf(alert) {
  if (!alert) return null;
...
function liveHazardsForCountry(countryName) {
...
function liveHazardsAggregated(countryNames) {
...
function enrichEventWithImpact(e) {
 ...
 */
function suggestTemplate(alert) {
...
function stripMessageAtts(m) {
  return { ...m, attachments: (m.attachments || []).map(stripAtt) };
}
function stripIncident(i) {
  // Strip Sprint-5 private bookkeeping fields before localStorage write —
  // _persistPending getting restored as 'true' on next boot would block
  // future API persists on this incident, _backendResponses is a transient
  // field used only during backfillIncidents hydration, and _pendingMessages
  // is the in-flight message-persist queue (would be stale on reload).
...
const TOKEN_KEY = 'nrsa-jwt';

function getStoredToken()  { try { return localStorage.getItem(TOKEN_KEY); } catch { return null; } }
function storeToken(tok)   { try { localStorage.setItem(TOKEN_KEY, tok); } catch {} }
...
function mapIncidentRowToState(r, sub = {}) {
  return {
    id:           r.id,
    title:        r.title,
...
const BACKEND_TYPE_TO_CATEGORY = {
...
const BACKEND_CATEGORY_TO_LABEL = {
...
function mapBackendCategory(evt) {
...
async function backfillWhoOutbreaks() {
 ...
 */
function isLocalIncidentId(id) {
  return typeof id === 'string' && id.startsWith('i_');
}

async function migrateLocalIncidents() {
  if (!API_BASE) return;
